...
<div class="flex min-h-screen" x-data="{ sidebarOpen: true }">
    <aside :class="sidebarOpen ? 'w-64' : 'w-16'" class="bg-green-900 text-white flex flex-col transition-all duration-300 min-h-screen">
        <div class="flex items-center justify-between p-4 border-b border-green-700">
            <span x-show="sidebarOpen" class="font-bold text-lg truncate">CUP Docente</span>
            <button @click="sidebarOpen = !sidebarOpen" class="text-green-300 hover:text-white focus:outline-none">
        ...
        <nav class="flex-1 py-4 overflow-y-auto">
            @php
                ...
                // El item de Notas se renderiza de forma especial (necesita parámetro dinámico)
            @endphp
            @foreach($items as $item)
            <a href="{{ route($item['route']) }}"
               class="flex items-center gap-3 px-4 py-2.5 hover:bg-green-700 transition-colors {{ request()->routeIs($item['route']) && $loop->first ? 'bg-green-700 border-l-4 border-green-300' : '' }}">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="bg-white shadow-sm px-6 py-4 flex items-center justify-between">
            <h1 class="text-xl font-semibold text-gray-800">@yield('page-title', 'Panel Docente')</h1>
            <div class="flex items-center gap-3">
                <span class="text-sm text-gray-600">{{ auth()->user()->nombre }} {{ auth()->user()->apellido }}</span>
                <span class="bg-green-100 text-green-800 text-xs font-medium px-2.5 py-0.5 rounded">Docente</span>
            </div>
        </header>
